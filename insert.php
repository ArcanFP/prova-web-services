...
$sql = "select * from trimonsgym where ntessera = " . $numeroTessera;
...
$sql = "select * from trimonsgym where email LIKE '" . $email . "'";
$result = ($conn->query($sql));
//declare array to store the data of database
...
if ($result->num_rows > 0) {
	...
	$sql = "INSERT INTO trimonsgym VALUES ('$nome',
	...
	if (mysqli_query($conn, $sql)) {
		...
		echo nl2br("\n$nome\n $cognome\n "
			. "$genere\n $email\n $numeroTessera");
		header("refresh:3;url=http://wpschool.it/palestre/Trimons-Gym/");
	} else {
		echo "Errore: $sql. "
			. mysqli_error($conn);
	}
